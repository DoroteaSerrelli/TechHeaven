...
import model.NavigazioneService;
...
public class NavigazioneController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet NavigazioneController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet NavigazioneController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      String productIdString = request.getParameter("id");  
        ...
        try {
        // Parse the productIdString to an integer
        int productId = Integer.parseInt(productIdString);
        
        //int codice = Integer.parseInt(request.getParameter("id"));        
        NavigazioneService productService = new NavigazioneService();
        Prodotto prodotto = productService.getProductByCodice(productId);
        request.setAttribute("prodotto", prodotto);
        request.getRequestDispatcher("Dettagli_Prodotto.jsp").forward(request, response);
        }
