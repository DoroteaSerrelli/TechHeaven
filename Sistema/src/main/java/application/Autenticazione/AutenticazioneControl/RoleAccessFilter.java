...
public class RoleAccessFilter implements Filter {
    ...
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
        ...
        if (debug) {
            log("RoleAccessFilter:DoBeforeProcessing");
        }
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("RoleAccessFilter:DoAfterProcessing");
        }
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        // Check if the user has the required role (e.g., catalog manager)
        ArrayList<String> rolesToCheck = new ArrayList<>();
        rolesToCheck.add("GestoreCatalogo");
        rolesToCheck.add("GestoreOrdini");
    ...
    private boolean checkUserRole(ProxyUtente user, ArrayList<String> rolesToCheck) {
        // Retrieve user roles from the user object
        ArrayList<Ruolo> userRoles = user.getRuoli();
        // Check if the user has the specified role
        for (String roleToCheck : rolesToCheck) {
            for (Ruolo role : userRoles) {
                if (role.getNomeRuolo().equals(roleToCheck)) {
                    return true;
                }
            }
        }    
    ...
    public void init(FilterConfig filterConfig) {        
        ...
        if (filterConfig != null) {
            if (debug) {                
                log("RoleAccessFilter:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        if (filterConfig == null) {
            return ("RoleAccessFilter()");
        }
        StringBuffer sb = new StringBuffer("RoleAccessFilter(");
