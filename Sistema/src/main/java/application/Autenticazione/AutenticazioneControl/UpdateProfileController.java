...
public class UpdateProfileController extends HttpServlet {
	...
	public UpdateProfileController() throws ServletException {
		Context initContext;
		Context envContext;
		DataSource ds = null;
		try {
	...
	//Costrutto per test
	public UpdateProfileController(UtenteDAODataSource userDAO, AutenticazioneController loginController, AutenticazioneServiceImpl as) {
		...
		try { 
			...
			if (information.equalsIgnoreCase("email") && updated_email != null && !updated_email.isEmpty()) {
				...
				if(Cliente.checkValidateEmail(updated_email)) {
					...
					updated_user.setDAO(new UtenteDAODataSource());
